...
const AddressFromShema=Yup.object().shape({
  name: Yup.string().required("Name is required"),
  mobile: Yup.string().required("Mobile number is required").matches(/^[6-9]\d{9}$/,"INvalied Mobile Number"),
  pinCode: Yup.string().required("pin code is required").matches(/^[1-9][0-9]{6}$/,"Invalied Pin code"),
...
const AddressForm = () => {
  const formik=useFormik({
    ...
    validationSchema:AddressFromShema,
    onSubmit:(values)=>{
      console.log(values)
    },
  });
